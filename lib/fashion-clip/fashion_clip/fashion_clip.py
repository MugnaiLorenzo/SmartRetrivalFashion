...
class FashionCLIP:
    ...
    def __init__(self, model_name, dataset: FCLIPDataset = None, normalize=True, approx=True, auth_token=None):
        ...
        if normalize and dataset:
            ...
            self.textual_vectors = self.textual_vectors / np.linalg.norm(self.textual_vectors, ord=2, axis=-1,
                                                                         keepdims=True)
            print('Done!')
    ...
    def set_approx(self, approx):
        self.approx = approx
        if self.approx and self.dataset:
            print('Building Approx NN index...', end='')
            # build approx NN
            self.nn_index = AnnoyIndex(512, "dot")
            for idx, v in enumerate(self.image_vectors):
    ...
    def retrival_img(self, image: List[str], top_k: int = 10):
        """
                Image retrieval from image
                :return:
                """
        # encode image
        img_vectors = self.encode_images(image, batch_size=8)
        # compute cosine similarity
        # cosine_sim = self._cosine_similarity(text_vectors, self.image_vectors)
        return self._nearest_neighbours(k=top_k, key_vectors=img_vectors, space_vectors=self.image_vectors)
